...
def generate_area_html(df: pd.DataFrame) -> str:
    ...
    for idx, row in df.iterrows():
        ...
        # Location
        lines.append("            <td style=\"" + style_str + "\">" + str(row["Location"]) + "</td>")
        # Projection (Units)
        ...
        lines.append("            <td style=\"" + style_str + "\">" + proj_content + "</td>")
        # Transactions (Units)
        ...
        lines.append("            <td style=\"" + style_str + "\">" + trans_content + "</td>")
        # SMAPE Value (%)
        smape = row.get('SMAPE Value (%)', np.nan)
        smape_val = "Unavailable" if pd.isna(smape) else smape
        lines.append("            <td style=\"" + style_str + "\">" + str(smape_val) + "</td>  <!-- SMAPE value -->")
        # Requested (Units)
        ...
        lines.append("            <td style=\"" + style_str + "\">" + req_content + "</td>")
        # Received (Units)
        ...
        lines.append("            <td style=\"" + style_str + "\">" + rec_content + "</td>")
        # Fulfillment Calculation (%)
        full = row.get('Fulfillment Calculation (%)', np.nan)
        full_val = 0 if pd.isna(full) else full
        full_content = str(full_val) + "%"
        lines.append("            <td style=\"" + style_str + "\">" + full_content + "</td>")
        # Out of Stock Count
        ...
        lines.append("            <td style=\"" + style_str + "\">" + out_content + "</td>")
        lines.append("        </tr>")
        # Add empty line after a row
        lines.append("        ")
