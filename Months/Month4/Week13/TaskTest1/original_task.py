...
def joint_probability(df, variables):
    if not variables:  # If no variables are provided, return the sum of all probabilities
        return {'': df['P'].sum()}
...
marginal_A = marginal_probability(df, 'A')
marginal_B = marginal_probability(df, 'B')
marginal_C = marginal_probability(df, 'C')
marginal_D = marginal_probability(df, 'D')

# Calculate joint probabilities
joint_AB = joint_probability(df, ['A', 'B'])
joint_AC = joint_probability(df, ['A', 'C'])
joint_AD = joint_probability(df, ['A', 'D'])
joint_BC = joint_probability(df, ['B', 'C'])
joint_BD = joint_probability(df, ['B', 'D'])
joint_CD = joint_probability(df, ['C', 'D'])
joint_ABC = joint_probability(df, ['A', 'B', 'C'])
joint_ABD = joint_probability(df, ['A', 'B', 'D'])
joint_ACD = joint_probability(df, ['A', 'C', 'D'])
joint_BCD = joint_probability(df, ['B', 'C', 'D'])
