...
class TaskSuite:
    def __init__(self):
        self.node = None  # This will be set externally

    def construct_worker_payload(self, worker, nonce):
        ctx = {}  # simulate context.Background()

        if worker.get('InferenceEntrypoint') is None and worker.get('ForecastEntrypoint') is None:
        ...
        if worker.get('InferenceEntrypoint') is not None:
        ...
        if worker.get('ForecastEntrypoint') is not None:
            ...
            if forecasts is None:
                ...
                return False, "Error computing forecast"
            worker_response['ForecasterValues'] = forecasts

        worker_response['TopicId'] = worker.get('TopicId')
        ...
        if self.node['wallet']['submit_tx']:
            _, err = self.node['send_with_retry'](ctx, req, "Transmit Worker Data to chain")
            if err is not None:
    ...
    def construct_payload(self, worker_response, height):
        ...
        if 'ForecasterValues' in worker_response and len(worker_response['ForecasterValues']) > 0:
            elements = []
            for val in worker_response['ForecasterValues']:
                ...
                if err is not None:
                    ...
                    return {}, err
                if not worker_response['WorkerConfig'].get('AllowsNegativeValue', False):
    ...
    def secure_worker_payload(self, payload):
        proto_bytes = b""  # simulate create byte slice
    ...
    def convert_to_dec(self, value):
        # This method is supposed to be mocked in tests
        pass
    ...
    def simulate_marshal(self, proto_bytes, payload, deterministic):
        # This method is supposed to be mocked in tests
        pass