...
class QuizMaster:
    ...
    def validate_input(self, event=None):
        user_input = self.entry_box.get().strip().lower().replace(" ", "")
        correct_word = self.current_word.strip().lower().replace(" ", "")
        if user_input == correct_word:
            print("Well done!")
            self.correct_answers += 1
            # Use the original case of the word when deleting from vocab
            original_word = next(word for word in self.vocab.keys() if word.lower().replace(" ", "") == correct_word)
            del self.vocab[original_word]
            self.refresh_display()
        else:
            print("Try again!")
