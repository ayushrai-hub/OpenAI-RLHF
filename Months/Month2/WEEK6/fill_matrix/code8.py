    ...
    for i in range(n):
        ...
        if np.any(missing):
            ...
            for k in range(K):
                ...
                var_k_observed = var[k][observed]

                # Compute log-likelihood for the observed data
                log_likelihood[k] = np.log(pi[k]) - 0.5 * np.sum((diff ** 2) / var_k_observed)
                log_likelihood[k] -= 0.5 * np.sum(np.log(2 * np.pi * var_k_observed))
