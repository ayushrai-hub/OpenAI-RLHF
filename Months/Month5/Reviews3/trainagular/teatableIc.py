...
def create_spectrum(draw, x_start, y_start, width, height, color_start, color_end, current_value):
    ...
    # Calculate the marker coordinates (rotated 180 degrees)
    marker_x = x_start  # Move marker to the left of the color scale
    marker_y = y_start + height - (current_value / 3000) * height
    marker_coords = [(marker_x + 10, marker_y - 5),
                     (marker_x + 20, marker_y),
                     (marker_x + 10, marker_y + 5)]

    # Add the marker
    draw.polygon(marker_coords, fill='black')

    # Return marker coordinates for unit testing
