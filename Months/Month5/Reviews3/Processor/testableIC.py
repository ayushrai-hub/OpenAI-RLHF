class IntegratedProcessor(object):
    """
    Integrates features of both Processor and ScenarioSimulator.
    
    PURPOSE: Extends from distributor with optional scenario functionality such as path drawing.
    ...
    """
    def __init__(self, PROP, phi_init='env_default', mass=1, prevent_dwell=True, name='PROCESSOR', 
                 out_dir='processing', scenario=False, **kwargs):
        self.PROP = PROP
        self.scenario = scenario  # Marker for scenario features
        if phi_init == 'env_default':
            self.phi_init = self.handle_phi(self.ENV.start, self.n_state)
        else:
            self.phi_init = self.handle_phi(phi_init, self.n_state)
    ...
    def progress(self, n_step=1, phi_init=None, overlook_imag=True):
        ...
        for i in range(n_step):
            ...
            phi_hi = self.normalize_density(phi_hi, gamma=self.gamma, type='l1')
            phi_init = phi_hi
        self._validate_state_density(phi_state=phi_init)
        return phi_init
    ...
    def _validate_state_density(self, phi_state, decimal_accuracy=config.decimal_accuracy):
        ...
        if np.allclose(phi_state.sum(), 1):
            ...
            raise ValueError(f'PROCESSOR: State density sum equals {phi_state.sum():.8f} (!= 1).')
    ...
    def _state_sample(self, phi_state, past_states=None):
        ...
        return self.sample_entity(phi_state)
    ...
    def generate_series(self, n_step=100, n_samp=50, phi_start=None, quick_store=True):
        ...
        phi_start = self.handle_phi(phi_start, self.n_state)
        seq_steps_total = n_step + 1
        self.n_step = n_step
        self.n_samp = n_samp
        ...
        iterator = tqdm(range(n_samp), desc='GENERATING') if config.verbose else range(n_samp)
        
        for ns in iterator:
            current_state = self._state_sample(phi_start)
            state_series[ns, 0] = current_state
            phi_vals[ns, 0, :] = phi_start
            ...
            phi_interim = self.handle_phi(current_state, self.n_state)
    ...
    def plot_path(self, samp=0):
        if self.scenario:
            ...
            raise AttributeError("Scenario functionality is disabled.")
    ...
    def calculate_cf(self, axis=None, dist_occ=0, zero_pos=False):
        if self.scenario:
            ...
            self.acf_mean, self.acf_sem = self.estimate_scenario_acf_v2(sampled_state, axis=axis)
        else:
            sampled_state = self._retrieve_state(samp=None, step=None, coords=False)
            if zero_pos:
                self.acf_mean, self.acf_sem = self.estimate_occ_acf_zero(sampled_state.T, d=dist_occ)
            else:
                self.acf_mean, self.acf_sem = self.estimate_occ_acf(sampled_state.T, d=dist_occ)
