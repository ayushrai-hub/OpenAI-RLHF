...
def simulate_spin_hamiltonian(N: int = 2, B: float = 4.5, phi_deg: float = 30) -> None:
    """
    Simulates a quantum spin system with an electron and N nuclear spins in a magnetic field.
    
    Parameters:
        N (int): Number of nuclear spins in the system (default: 2).
        B (float): Magnetic field strength in Tesla (default: 4.5 T).
        phi_deg (float): Tilt angle of the magnetic field in degrees (default: 30°).
    
    Returns:
        None
    """
    # Electron g-factor (for GaAs)
    g_e = -0.44
    ...
    tlist = np.linspace(0, 1e-6, 500)  # Time in seconds
    
    # Observables (electron spin projection)
    obs = [Sx_total, Sy_total, Sz_total]
    
    # Time evolution of the system
    result = qt.mesolve(H, initial_state, tlist, [], obs)
    
    # Plotting the spin dynamics
    plt.figure(figsize=(8, 6))
    plt.plot(tlist * 1e6, result.expect[0], label=r'$\langle S_x \rangle$')
    plt.plot(tlist * 1e6, result.expect[1], label=r'$\langle S_y \rangle$')
    plt.plot(tlist * 1e6, result.expect[2], label=r'$\langle S_z \rangle$')
    plt.xlabel('Time (µs)')
