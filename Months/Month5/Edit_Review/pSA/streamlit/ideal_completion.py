...
try:
    ...
    # Mock _snowflake for testing purposes
    class MockSnowflake:
        @staticmethod
        def send_snow_api_request(*args, **kwargs):
            return {
                "status": 200,
                "content": '{"message": {"content": [{"type": "text", "text": "Mock response"}]}}'
            }
    _snowflake = MockSnowflake()
import json
...
def show_content(content: list, message_index: int = None) -> None:
    ...
    message_index = message_index if message_index is not None else len(st.session_state.messages)
    for item in content:
        if item["type"] == "text":
