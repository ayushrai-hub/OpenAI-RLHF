...
def l4(raw_input: str, plot: bool) -> str:
    ...
    while lines:
        ...
        if nx.is_strongly_connected(walkable.graph):
            hamiltonian_path = nx.hamiltonian_path(walkable.graph)
            # We got a proper Hamiltonian path, convert this to WASD
            path_string = translate_path_to_directions([node for node, in hamiltonian_path])
        else:
            raise ValueError("Given graph structure doesn't allow Hamiltonian traversal or basic node constraints need adjustment.")
