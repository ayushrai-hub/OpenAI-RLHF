...
# Connect precision metrics and annotate explicitly with scalar conversion
for label, precision in precision_metrics.items():
    x_pos = label
    y_pos = float(precision)
    ax2.plot(x_pos, y_pos, marker='o', color='blue')
    ax2.text(x_pos, y_pos, f'Precision: {y_pos:.2f}', ha='center', va='bottom', color='blue')

# Connect recall metrics and annotate explicitly with scalar conversion
for label, recall in recall_metrics.items():
    x_pos = label
    y_pos = float(recall)
    ax2.plot(x_pos, y_pos, marker='o', color='green')
    ax2.text(x_pos, y_pos, f'Recall: {y_pos:.2f}', ha='center', va='bottom', color='green')
