...
class MyApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        load_button = ttk.Button(self.button_frame, text="Import Transaction Records", command=self.load_records, style="TButton")
        load_button.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

        show_graph_button = ttk.Button(self.button_frame, text="Show Graph", command=self.show_graph, style="TButton")
        show_graph_button.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

        separator = ttk.Separator(main_frame, orient='horizontal')
        separator.pack(fill='x', padx=2, pady=2)

        # Define Data interface specific widgets
        self.data_interface_frame = ttk.Frame(main_frame)
        self.data_interface_frame.pack(pady=10, fill=tk.X)
        self.configure_data_interface()

        # Placeholder for Graph interface specific widgets
        self.graph_interface_frame = ttk.Frame(main_frame)
        self.graph_interface_frame.pack_forget()  # Initially hidden

    def configure_data_interface(self):
        search_var = tk.StringVar()
        search_box = ttk.Entry(self.data_interface_frame, textvariable=search_var, width=50)
        ...
        uncategorized_check = ttk.Checkbutton(self.data_interface_frame, text="Uncategorized Only", variable=uncategorized_var)
        uncategorized_check.grid(row=0, column=1, sticky="w")

        self.tree = ttk.Treeview(self.data_interface_frame, columns=("Transaction", "Purchase Date", "Post Date", "Amount", "Details", "Balance"), show="headings", style="Treeview")
        ...
        self.category_combobox = ttk.Combobox(self.data_interface_frame, textvariable=self.category_var, style="TCombobox")
        self.category_combobox.pack(pady=2, fill=tk.X)
        self.category_combobox.bind('<KeyRelease>', self.filter_category_options)

        categorize_button = ttk.Button(self.data_interface_frame, text="Assign Category", style="TButton")
        categorize_button.pack(pady=2, fill=tk.X)

    def show_graph(self):
        # Toggle view logic
        self.data_interface_frame.pack_forget()
        self.graph_interface_frame.pack(pady=10, fill=tk.BOTH, expand=True)
        messagebox.showinfo("Information", "Graph display functionality to be implemented here.")
