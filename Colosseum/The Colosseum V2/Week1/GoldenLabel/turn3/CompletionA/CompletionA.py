...
# Convert label into a scalar type before plotting precision and recall values
for label, precision in precision_metrics.items():
    label_str = str(label)  # Ensure label is a string
    ax2.plot([label_str], [precision], marker='o', color='blue')
    ax2.text(label_str, precision, f'Precision: {precision:.2f}', ha='center', va='bottom', color='blue')

for label, recall in recall_metrics.items():
    label_str = str(label)  # Ensure label is a string
    ax2.plot([label_str], [recall], marker='o', color='green')
    ax2.text(label_str, recall, f'Recall: {recall:.2f}', ha='center', va='bottom', color='green')
